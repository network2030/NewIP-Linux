...
int xdp_router_func(struct xdp_md *ctx)
{
	...
	else if (h_proto == bpf_htons(ETH_P_NEWIP))
	{
		newiph = data + nh_off;
		...
		// Check pkt contents at /sys/kernel/debug/tracing/trace_pipe
		bpf_printk("total size: %d\n", data_end - data);
		bpf_printk("eth size: %d\n", sizeof(*eth));
		bpf_printk("newip size: %d\n", sizeof(*newiph));
		bpf_printk("addr cast: %d\n", newiph->addr_cast);
		bpf_printk("src addr type: %d, src addr :%d\n", newiph->src_addr_type, bpf_ntohl(newiph->src));
		bpf_printk("dst addr type: %d, dst addr :%d\n", newiph->dst_addr_type, bpf_ntohl(newiph->dst));
		bpf_printk("dummy: %d\n", newiph->dummy);
		bpf_printk("\n");
